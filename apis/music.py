...
class WYYMSC_API:
    def __init__(self):
        self.js_code = execjs.compile(jscode)
        self.csrf_token = '046a07c4cb59ef4c82571fe260e3b36b'
    ...
    def encrypt(self,data):
        ...
        g = '0CoJUm6Qyw8W8jud'
        return self.js_code.call('d', d, e, f, g)
    ...
    def search(self,keyWord,type=0,offset=0,limit=99):
        ...
        if type == 0:
            ...
            list = {'s': keyWord, 'limit': '16', 'type': '1', 'csrf_token': self.csrf_token}
        else:
            ...
            list = {
                ...
                'type': type,
                'offset': offset,
                'limit': limit, # 只查询前20条信息
                'csrf_token': self.csrf_token
            }
        return url,list

    #mv 信息
    def target_mv_info(self, id):
        url = 'http://music.163.com/weapi/mv/detail'
        list = {'id': id, 'type': 'mp4'}
        return url,list

    #歌曲 信息
    def target_songs_info(self, id):
        str = '[{"id":"%s"}]' % id
        url = 'http://music.163.com/api/v3/song/detail?id=%s&c=%s' % (id, urlparse.quote(str))
        list = {'csrf_token':self.csrf_token}
        return url,list

    def target_songs_downloadlink(self, ids,br):
    ...
    def search_sq(self, key):
        return 'http://sug.music.baidu.com/info/suggestion?word=%s&version=2&from=0' % key
    def search_sq_songid(self,id):
        return 'http://music.baidu.com/data/music/fmlink?songIds=%s&type=flac' % id